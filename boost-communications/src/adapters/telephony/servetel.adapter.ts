...
export class ServetelTelephonyAdapter implements IVoiceAdapter {
  public providerName = 'servetel';
  ...
  public async call(options: VoiceCallOptions): Promise<UniversalResult> {
    ...
    const res = await safeFetchJson('https://api.servetel.in/v1/click_to_call', {
      method: 'POST',
      headers: {
    ...
    return {
      ...
      messageId: res.data?.call_id || res.data?.data?.call_id,
