...
import { BoostPaymentOpenOptions, GatewayName, UnifiedOrderResult } from '../types';
 ...
 */
export async function openPaymentModal(options: BoostPaymentOpenOptions): Promise<void> {
  ...
  if (gateway === 'cashfree') {
    if (order.paymentSessionId) {
      ...
      if (CashfreeConstructor) {
        const isSandbox = order.paymentSessionId.includes('test') || (order.rawResponse as any)?.environment === 'SANDBOX';
