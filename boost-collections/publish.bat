@echo off
echo ========================================================
echo  Boost Engine Collections - 1-Click NPM Publish
echo ========================================================
echo.

cd /d "%~dp0"

echo [1/3] Building and validating collections...
call npm run build
if %errorlevel% neq 0 (
    echo.
...
if %errorlevel% neq 0 (
    echo.
    echo [!] Tests failed. Postman schema validation did not pass.
    echo.
    pause
    exit /b 1
)
...
echo.
echo [3/3] Publishing @boostengine/collections to npm publicly...
