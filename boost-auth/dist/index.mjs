...
var TokenManager = class {
  ...
  sign(payload, expiresInSeconds) {
    ...
    const signature = crypto.createHmac("sha256", this.secret).update(`${encodedHeader}.${encodedPayload}`).digest("base64").replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_");
  ...
  verify(token) {
    try {
      ...
      const expectedSignature = crypto.createHmac("sha256", this.secret).update(`${encodedHeader}.${encodedPayload}`).digest("base64").replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_");
...
var AuthRouter = class {
  ...
  async handleSendEmailOtp(request) {
    ...
    const randomBytes6 = crypto.randomBytes(length);
...
var TOTPManager = class {
  ...
  static generateSecret(byteLength = 20) {
    const randomBytes6 = crypto.randomBytes(byteLength);
